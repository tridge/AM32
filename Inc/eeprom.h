...
typedef union EEprom_u {
    struct {
        ...
        uint8_t brake_on_zero_throttle; //13  behavior when throttle is 0 but motor is still rotating
                                        //    0 off, 1 coast, 2 brake, 3-9 coast for (value - 2)
                                        //    seconds then hand over to brake_on_stop
